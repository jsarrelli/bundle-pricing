...
/**
 * Contains implementation of the domain, including The cart, cart items, price & quantity units, and the logic to
...
object BundlePricingDomain {
  ...
  sealed trait BundlePromotion {
    def cartItems
      : Seq[CartItem]               // the items of the bundle, example 2 apples + 1 margarine, it's how we detect the bundle pattern
  ...
  object Price {
    def apply(value: Int): Price =
      if (value >= 0) new Price(value) else throw new Exception(value + " isn't a valid Price : 0 <= Integer")
  }
...
object BundlePromotions {
  ...
  /**
   * Discount on total price example 1 apple $2.00 , 2 apples for $3.00
   */
  ...
  /**
   * Discount on specific item(s) (we override the unit price) into a promotion example a loaf of bread “A” purchased
  ...
  class BundleDiscountOnItemUnitPrice(discountedItems: Seq[MaybeDiscountedItem]) extends BundlePromotion {
    ...
    def totalDiscountedPrice = {
      val itemsPrices = discountedItems.map { itemWrapper =>
        itemWrapper.optionalUnitPriceOverride match {
  ...
  object BundleTotalPriceDiscount {
    def apply(cartItemsParams: Seq[CartItem], totalPrice: Price) =
      new BundleTotalPriceDiscount(cartItemsParams, totalPrice)
  }
...
object BundlePricingUtil {
  ...
  def groupCartItem(cartItems: Seq[CartItem]): Seq[CartItem] = {
    ...
    cartItems
      .groupBy(_.catalogItem)
      .map { case (item, toGroupItems) =>
        CartItem(item, Quantity(toGroupItems.map(_.quantity.value).sum))
      }
      .toSeq
  }
